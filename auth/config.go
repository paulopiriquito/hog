...
func FromYAML(node yaml.Node) (Config, error) {
	...
	c := Config{
		...
		UserInfo:   orDefault(raw.UserInfo, "auto"),
	}
	switch c.UserInfo {
	case "auto", "always", "never":
	default:
		return Config{}, fmt.Errorf("auth: userInfo must be auto|always|never (got %q)", c.UserInfo)
	}
	return c, nil
}
...
// NeedsUserInfo decides whether the callback must call the userinfo endpoint.
func NeedsUserInfo(sess session.Config, mode string) bool {
	switch mode {
	case "always":
		return true
	case "never":
	...
	default: // auto
		...
		for _, c := range sess.PassportClaims {
